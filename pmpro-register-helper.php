...
/*
	Includes
*/
require_once( PMPRORH_DIR . '/shortcodes/pmpro_signup.php' );
// [pmpro_signup ...] shortcode
/*
	Loading Modules
...
/*
	...
	// textarea
	$history = new PMProRH_Field(
		'history', 'textarea', array(
			'rows' => 10,
			'label' => 'Tell us a little about your history.',
		)
	);

	// hidden
	$secret = new PMProRH_Field(
		'secret', 'hidden', array(
 ...
 */
	...
	pmprorh_add_registration_field( 'after_billing_fields', $text );
		...
		/**
		 * Add a field to the PMProRH regisration fields globa l
	 ...
	 */
		...
		if ( ! empty( $addmember_fields ) ) {
			...
			foreach ( $addmember_fields as $field ) {
				...
				} elseif ( ! empty( $_POST[ $field->name . '_checkbox' ] ) && $field->type == 'checkbox' ) {
					// callback?
					if ( ! empty( $field->save_function ) ) {
	...
	*/
	 ...
	 * Adds text area for emails and user names on edit level page.
	 */
